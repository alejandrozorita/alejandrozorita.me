...
/***/ "./resources/js/jquery.gridrotator.js":
/*!********************************************!*\
  !*** ./resources/js/jquery.gridrotator.js ***!
  \********************************************/
  ...
  */
  ...
  jQuery.GridRotator.prototype = {
    _init: function _init(options) {
      // options
      this.options = jQuery.extend(true, {}, jQuery.GridRotator.defaults, options); // cache some elements + variables
    ...
    _config: function _config() {
      ...
      this.transEndEventName = transEndEventNames[Modernizr.prefixed('transition')] + '.gridrotator'; // all animation types for the random option
      ...
      this.animTypesTotal = this.animTypes.length; // the <ul> where the items are placed

      this.jQuerylist = this.jQueryel.children('ul'); // remove images and add background-image to anchors
      ...
      jQueryimgs.each(function () {
        ...
        jQuery('<img/>').load(function () {
          ...
          if (loaded === count) {
            jQueryimgs.remove();
            self.jQueryel.removeClass('ri-grid-loading'); // the items

            self.jQueryitems = self.jQuerylist.children('li'); // make a copy of the items

            self.jQueryitemsCache = self.jQueryitems.clone(); // total number of items

            self.itemsTotal = self.jQueryitems.length; // the items that will be out of the grid
    ...
    _layout: function _layout(callback) {
      var self = this; // sets the grid dimentions based on the container's width

      this._setGridDim(); // reset
      ...
      jQueryoutItems.remove(); // container's width
    ...
    _setGridDim: function _setGridDim() {
      // container's width
      var c_w = this.jQueryel.width(); // we will choose the number of rows/columns according to the container's width and the values set in the plugin options 
    ...
    _initEvents: function _initEvents() {
      ...
      var visProp = getHiddenProp(); // HTML5 PageVisibility API
      // http://www.html5rocks.com/en/tutorials/pagevisibility/intro/
      // by Joe Marini (@joemarini)
    ...
    _getAnimType: function _getAnimType() {
      return this.animType === 'random' ? this.animTypes[Math.floor(Math.random() * this.animTypesTotal)] : this.animType;
    },
    // get css properties for the transition effect
    _getAnimProperties: function _getAnimProperties(jQueryout) {
      var startInProp = {},
          startOutProp = {},
          endInProp = {},
          endOutProp = {},
    ...
    _showNext: function _showNext(time) {
      ...
      this.playtimeout = setTimeout(function () {
        ...
        for (var i = 0; i < nmbOut; ++i) {
          // element to go out
          var jQueryout = self.jQueryitems.eq(randArr[i]); // if element is active, which means it is currently animating,
    ...
    _replace: function _replace(jQueryout) {
      ...
      jQueryout.data('active', true); // get the element (anchor) that will go in (first one inserted in this.outItems)

      var jQueryinA = this.outItems.shift(); // save element that went out

      this.outItems.push(jQueryoutA.clone().css('transition', 'none')); // prepend in element
    ...
    _getRandom: function _getRandom(cnt, limit) {
      var randArray = [];
...
/***/ 5:
/*!**************************************************!*\
  !*** multi ./resources/js/jquery.gridrotator.js ***!
  \**************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /Applications/MAMP/htdocs/alejandrozorita.me/resources/js/jquery.gridrotator.js */"./resources/js/jquery.gridrotator.js");
