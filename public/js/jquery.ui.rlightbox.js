...
/***/ "./resources/js/jquery.ui.rlightbox.js":
/*!*********************************************!*\
  !*** ./resources/js/jquery.ui.rlightbox.js ***!
  \*********************************************/
...
(function ($, undefined) {
  $.widget("ui.rlightbox", {
    ...
    _create: function _create() {
      var _setElement,
          ...
          self = this; // init the lightbox – do in only once


      global.getLightbox(); // which type content belongs to: youtube, vimeo, flash, image, etc.
      // what is its url, title (for images), etc …

      _setElement = global.extractAnchor(this); // add type, url, jQuery element and title of content to a set if content is supported by the lightbox
      // otherwise fall silently

      if (_setElement.type !== undefined) {
        global.addToSet(_setElement); // open the lightbox upon click
  ...
  $.extend($.ui.rlightbox, {
    global: {
      addToSet: function addToSet(setElement) {
      ...
      checkButtonsState: function checkButtonsState() {
        var data = this.data,
            ...
            _isLoop = data.currentSetElement.self.options.loop; // if lightbox is opened and there is only one element
        ...
        if (_currentSet === "single" || _totalElements === 1) {
          ...
          this.setButtonState("disabled", $lb.prev); // when there are only two elements in a set

          this.setButtonState("default", $lb.next);
        } else if (_currentElement === _totalElements && _isLoop === false) {
          // in case of last element
          this.setButtonState("disabled", $lb.next); // when there are only two elements in a set
      ...
      checkMinimalSize: function checkMinimalSize(size, number) {
      ...
      closeLightbox: function closeLightbox() {
        ...
        if (data.ready) {
          $lb.overlay.add($lb.root).add($lb.header).hide();
          $("body").css("overflow", "visible"); // remove content and restore its initial size

          $lb.content.empty().width(20).height(20); // hide arrow cue

          this.hideArrow(); // reset control buttons states to default

          this.setButtonState("default"); // reset panorama

          this.panoramaHideIcon(); // reset the counter

          data.currentElementNumber = null;
          data.totalElementsNumber = null; // remove old title

          $lb.title.empty(); // hide the map

          this.panoramaHideMap(); // lightbox is not ready again

          data.ready = false;
        }
      },
      createStructure: function createStructure() {
        var data = this.data; // append the structure

        $(data.htmlLightbox).appendTo("body").after(data.htmlMap).after(data.htmlOverlay);
      },
      destroy: function destroy() {
        // unwrap $currentElement from jQuery wrapped object and
        // prevents it from being acted upon, unbinding event handlers
        var data = this.data,
            $currentElement = data.currentSetElement.element; // code taken from jqury.ui.widget.js – it is the default behaviour
        // from the widget factory but we can’t call it because it acts upon
        // this.element – we have to act on a arbitrary one

        $currentElement.unbind("." + "rlightbox").unbind("click").removeData("rlightbox");
      },
      extractAnchor: function extractAnchor(thisElement) {
        ...
        $.each(_service, function (name, content) {
          var _found = false; // outer loop: _content.youtube, _content.image, etc.

          $.each(content.urls, function (index, regExp) {
            var _res = regExp.exec(_url); // inner loop: urls array
      ...
      getAvailableScreenSize: function getAvailableScreenSize() {
        var data = this.data,
      ...
      getCurrentElementNumber: function getCurrentElementNumber(element) {
        var _currentNumber,
            data = this.data,
            sets = this.sets,
            self = this,
            _currentSet = sets[data.currentSet]; // returns a 1 based ordinal number of an image in a set


        $.each(_currentSet, function (i, v) {
          // compare DOM elements
          if ($(element).get(0) === v.element.get(0)) {
            _currentNumber = i + 1; // exit $.each()
      ...
      getImageStatus: function getImageStatus(width, height) {
      ...
      getLightbox: function getLightbox() {
        var data = this.data,
            $lb = this.$lightbox,
            self = this; // do it only once!

        if (!$lb.root) {
          // create the DOM structure of the lightbox
          this.createStructure(); // remember references to lightbox structure

          this.setReferences(); // close the lightbox upon clicking on the close button and the overlay

          $lb.close.add($lb.overlay).click($.proxy(this.closeLightbox, this)); // goes to the next element when button is clicked

          $lb.next.click($.proxy(this.next, this)); // and goes to the prev element when prev button is clicked

          $lb.prev.click($.proxy(this.prev, this)); // highlight buttons when mouse hovers over them

          $lb.next.add($lb.prev).add($lb.next) //.add( $lb.play )
          .add($lb.close).hover(function () {
          ...
          $lb.contentContainer.mousemove($.proxy(this.showArrow, this)).mousemove($.proxy(this.checkSide, this)).mousemove($.proxy(this.setCursor, this)).click(function () {
            if (data.side === "left") {
              self.prev.apply(self);
            } else if (data.side === "right") {
              self.next.apply(self);
            }
          }).mousedown($.proxy(this.panoramaStart, this)).mouseup($.proxy(this.panoramaStop, this)).mouseleave(function () {
            self.hideArrow.apply(self);
            data.side = "";
          }); // zoom in or zoom out an image

          $lb.panoramaIcon.click($.proxy(this.panoramaToggle, this)).hover($.proxy(this.panoramaHighlight, this)); // resize lightbox when window size changes

          $(window).bind("resize.rlightbox", $.proxy(this.liveResize, this)); // keyboard navigation

          $(document).keyup($.proxy(this.handleKeyboard, this));
        }
      },
      getOptimalSize: function getOptimalSize(size, number) {
      ...
      getParam: function getParam(param, url) {
      ...
      getSetName: function getSetName(thisElement) {
      ...
      checkSide: function checkSide(event) {
        var data = this.data,
            $lb = this.$lightbox,
      ...
      getSizes: function getSizes() {
        ...
        function _calculateSizes(w, h) {
          ...
          _statusHeight = _statuses.statusHeight; // if image fits the window
          ...
          if ((_statusWidth === 1 || _statusWidth === -1) && _statusHeight !== 2 && (_statusHeight === 1 || _statusHeight === -1) && _statusWidth !== 2) {
            ...
            if (_statusWidth === 1 || _statusWidth === -1) {
              ...
              if (w > h) {
                ...
                _imageTargetHeight = _lightboxTargetHeight; // if after scaling an image is smaller or bigger
      ...
      getWindowSize: function getWindowSize(size) {
        var data = this.data,
      ...
      liveResize: function liveResize() {
        var data = this.data,
            self = this,
            $lb = this.$lightbox,
            _elementType = data.currentSetElement.type; // resizes an image when size of the browser window resizes and when Panorama is turned off
      ...
      loadContentFlash: function loadContentFlash(url) {
        var _width,
            _height,
            $contentWrapper,
            data = this.data,
            ...
            _currentElement = data.currentSetElement,
            _options = _currentElement.self.options; // show the spinner
        ...
        function _load() {
          ...
          _height = self.getParam("height", url); // if &width and &height are invalid, use a default one
          ...
          _currentElement.height = _height; // use real data
          ...
          $content.removeClass("ui-lightbox-loader").empty().append(_structure).children().wrap($contentWrapper);
      ...
      loadContentImage: function loadContentImage(url) {
        var $lb = this.$lightbox,
            data = this.data,
            self = this,
            _currentElement = data.currentSetElement,
            _dfd = $.Deferred(),
            $newImage = $("<img />"); // show spinner


        $lb.content.addClass("ui-lightbox-loader");
        $newImage.attr("src", url).bind("load", function () {
          $(this).unbind("load"); // keep original size of an image – needed when resizing

          _currentElement.width = this.width;
          _currentElement.height = this.height; // add the loaded image and hide it

          $lb.content.removeClass("ui-lightbox-loader").empty().append(this).children().hide(); // continue the animation queue

          _dfd.resolve();
        }).error(function () {
          $lb.content.removeClass("ui-lightbox-loader");
          self.showErrorMessage(); // continue the animation queue
      ...
      loadContentVimeo: function loadContentVimeo(url) {
        var _width,
            _height,
            data = this.data,
            $lb = this.$lightbox,
            ...
            _currentElement = data.currentSetElement,
            _options = _currentElement.self.options,
            _minimalLightboxSize = data.minimalLightboxSize; // show loader
        ...
        $.ajax({
          ...
          // add embedded code
          $lb.content.removeClass("ui-lightbox-loader").empty().append(data.html).children().wrap("<div style='display: none'></div>").end().find("div:first").width(data.width).height(data.height); // remember video title
          ...
          _currentElement.height = data.height; // continue the animation queue
      ...
      loadContentYoutube: function loadContentYoutube(url) {
        var $contentWrapper,
            data = this.data,
            $lb = this.$lightbox,
            $content = $lb.content,
        ...
        $.ajax({
          ...
          $content.removeClass("ui-lightbox-loader").empty().append(_structure).children().wrap($contentWrapper); // remember video title
      ...
      navigationGoToElement: function navigationGoToElement(number) {
        // goes to a custom element
        var data = this.data,
            sets = this.sets,
            $lb = this.$lightbox,
            _currentSet = data.currentSet; // which element go to

        data.currentElementNumber = number;
        data.currentSetElement = sets[_currentSet][number - 1]; // reload animation queue and trigger it
      ...
      next: function next() {
        ...
        if (_isReady && _set !== "single" && _isPanoramaOn === false) {
          ...
          if (_play) {
            data.currentSetElement = sets[_set][_currentElementNumber - 1]; // next element - trigger the queue ‘next’ - first update it
      ...
      open: function open(thisElement) {
        var data = this.data,
            sets = this.sets,
            $lb = this.$lightbox,
        ...
        _currentSet = this.getSetName(thisElement),
            _currentUrl = $(_jqElement).attr("href"); // remember which set content belongs to


        data.currentSet = _currentSet; // determine and remember how many elements belong to a set
        // determine the current (and clicked) element in a set

        data.totalElementsNumber = sets[_currentSet].length;
        data.currentElementNumber = this.getCurrentElementNumber(_jqElement); // keep a reference to a current element in a set (consisting of a url, type…)

        data.currentSetElement = sets[_currentSet][data.currentElementNumber - 1]; // set animation queues

        this.setOpenQueue();
        this.setNextQueue(); // to fade or not to fade…

        this.checkButtonsState(); // start opening the lighbox

        $lb.queueContainer.open.dequeue("lightboxOpen");
      },
      panoramaCenterContent: function panoramaCenterContent() {
        var _left,
            _top,
            data = this.data,
            $lb = this.$lightbox,
            ...
            _imageHeight = _currentElement.height,
            $content = $lb.content,
            $img = $content.find("img"); // if width of an image was bigger than the available screen space and if we divided the both size by two
      ...
      panoramaCheckAvailability: function panoramaCheckAvailability() {
        ...
        if (_currentImageWidth < _originalImageWidth || _currentImageHeight < _originalImageHeight) {
          this.panoramaShowIcon("expand"); // cuz we don’t want to check it twice in panoramaToggle()
      ...
      panoramaExpand: function panoramaExpand(event) {
        // _panoramaExpand does the main goal of the Panorama™: it displays the natural image size
        var data = this.data,
            $lb = this.$lightbox,
            _currentElement = data.currentSetElement,
            _options = _currentElement.self.options; // let know that we can scroll now

        data.panoramaOn = true; // show the zoom out icon; we add hover state because when we click
        ...
        $lb.content.css("overflow", "hidden"); // give the natural size to the image

        $lb.content.find("img").width(_currentElement.width).height(_currentElement.height); // enlarge lightbox’s content to fit the screen best

        this.panoramaSetContentSize(); // center the content and the whole lightbox

        this.panoramaCenterContent();
        this.queueCenterLightbox(); // update header width

        this.updateTitleWidth(); // show the map
        ...
        this.hideArrow(); // reset cursor when there is no movement; for example
        // cursor was ‘pointer’, [Z] buttons was pressed (‘default’ cursor)
        // [Z] was pressed again → cursor is still ‘pointer’

        this.setCursor();
      },
      panoramaHideIcon: function panoramaHideIcon() {
        var data = this.data,
            $lb = this.$lightbox;
        $lb.panoramaIcon.hide().removeClass("ui-lightbox-panorama-icon-expand ui-lightbox-panorama-icon-shrink");
        data.panoramaOn = false;
      },
      panoramaHideMap: function panoramaHideMap() {
        var $lb = this.$lightbox; // hide the map

        $lb.map.hide(); // reset position of the viewport
      ...
      panoramaHighlight: function panoramaHighlight() {
        var _suffixPosition,
            $lb = this.$lightbox,
            _suffix = "-hover";

        $lb.panoramaIcon.attr("class", function (index, oldValue) {
      ...
      panoramaSetContentSize: function panoramaSetContentSize() {
        var _contentWidth,
            _contentHeight,
            data = this.data,
            $lb = this.$lightbox,
            ...
            _screenHeight = _screenSize.height,
            _imageWidth = _currentElement.width,
            _imageHeight = _currentElement.height; // show the most part of an image
        ...
        $lb.content.width(_contentWidth).height(_contentHeight);
      },
      panoramaShowIcon: function panoramaShowIcon(icon, state) {
        var $lb = this.$lightbox,
            _state = state || "",
            _newClass = "ui-lightbox-panorama-icon-" + icon + _state;

        $lb.panoramaIcon.show().removeClass().addClass(_newClass);
      },
      panoramaShowMap: function panoramaShowMap() {
        var _mapViewportWidth,
            ...
            _currentElement = data.currentSetElement,
            _mapSize = data.mapSize; // show the map and give the viewport relevant size
        // give the viewport relevant size


        _mapViewportWidthRatio = _mapSize.width / _currentElement.width;
        _mapViewportHeightRatio = _mapSize.height / _currentElement.height; // content doesn't cover whole container
        ...
        $lb.viewport.width(_mapViewportWidth).height(_mapViewportHeight); // show the map

        $lb.map.show(); // used when you scroll the content
      ...
      panoramaShrink: function panoramaShrink(event) {
        var data = this.data,
            $lb = this.$lightbox; // _panoramaShrink retores the previous size of an image

        data.panoramaOn = false; // show the zoom out icon; we add hover state because when we click
        // the icon we lose focus and state end up with normal state
        // not when key is pressed
        ...
        this.queueResizeLightbox();
        this.queueCenterContent(); // fixes issue with Panorama in Firefox 3.0, 3.5, 3.6

        $lb.content.css("overflow", "visible"); // update header width

        this.updateTitleWidth(); // hide the map

        this.panoramaHideMap(); // reset cursor when there is no movement; for example
        // cursor was ‘pointer’, [Z] buttons was pressed (‘default’ cursor)
        // [Z] was pressed again → cursor is still ‘pointer’

        this.setCursor();
      },
      panoramaStart: function panoramaStart(event) {
        var data = this.data,
            $lb = this.$lightbox; // remember starting point to calculate distance in _panoramaStop()
      ...
      panoramaStop: function panoramaStop(event) {
        ...
        var data = this.data,
            ...
            $content = $lb.content,
            _viewportRatio = data.viewportRatio; // if we are in the panorama mode (the panorama icon was clicked)


        if (data.panoramaOn) {
          $content.scrollLeft($content.scrollLeft() + _distX).scrollTop($content.scrollTop() + _distY); // show the relevant part of the map
      ...
      panoramaToggle: function panoramaToggle(event) {
      ...
      removeSetElement: function removeSetElement(number) {
        ...
        var data = this.data,
            sets = this.sets,
            _currentSet = data.currentSet,
            _total = data.totalElementsNumber; // remove given element from a set

        sets[_currentSet].splice(number - 1, 1); // if there is only one element left, close the lightbox, otherwise load next element


        if (_total === 1 || _currentSet === "single") {
          this.closeLightbox(); // remove the instance from encapsulated DOM element (jquery one)

          this.destroy();
        } else {
          this.destroy(); // update total element numbers

          data.totalElementsNumber = sets[_currentSet].length; // go to a new element
      ...
      replaceHtmlPatterns: function replaceHtmlPatterns(htmlString, patterns) {
      ...
      setButtonState: function setButtonState(state, jqElement) {
        var $lb = this.$lightbox,
      ...
      setNextQueue: function setNextQueue() {
        // for description take a look at _setOpenQueue method
        var $lb = this.$lightbox,
            queueList = [$.proxy(this.queueSlideUpHeader, this), $.proxy(this.queueHideContent, this), $.proxy(this.queueLoadContent, this), $.proxy(this.queueResizeLightbox, this), $.proxy(this.queueCenterContent, this), $.proxy(this.queueShowContent, this), $.proxy(this.queueSlideDownHeader, this)]; // place start animation queue in the queue container

        $lb.queueContainer.next.queue("lightboxNext", queueList);
      },
      setOpenQueue: function setOpenQueue() {
        ...
        var $lb = this.$lightbox,
            queueList = [$.proxy(this.queueShowOverlay, this), $.proxy(this.queueCenterLightbox, this), $.proxy(this.queueLoadContent, this), $.proxy(this.queueResizeLightbox, this), $.proxy(this.queueCenterContent, this), $.proxy(this.queueShowContent, this), $.proxy(this.queueSlideDownHeader, this)]; // place start animation queue in the queue container

        $lb.queueContainer.open.queue("lightboxOpen", queueList);
      },
      setReferences: function setReferences() {
        var $lb = this.$lightbox; // save references to wrapped set for later use
        ...
        $lb.prev = $lb.root.find("#ui-lightbox-button-prev"); //$lb.play = $lb.root.find( "#ui-lightbox-button-play" );
      ...
      showArrow: function showArrow(event) {
        var data = this.data,
            $lb = this.$lightbox,
            $arrow = $lb.arrow,
            _isError = data.showErrorMessage,
            ...
            _currentElement = data.currentElementNumber,
            _totalElements = data.totalElementsNumber,
            _isLoop = data.currentSetElement.self.options.loop; // show arrow cues only in image set or in The Error Screen when it is part of a set
        ...
        if (data.ready && data.currentSet !== "single" && (data.currentSetElement.type === "image" || _isError) && data.panoramaOn === false) {
          if (_side === "left" && (_currentElement > 1 || _isLoop)) {
            $arrow.show().removeClass("ui-lightbox-arrow-next ui-corner-left").addClass("ui-lightbox-arrow-prev ui-corner-right").find("span").removeClass("ui-icon-carat-1-e").addClass("ui-icon-carat-1-w");
          } else if (_side === "right" && (_currentElement < _totalElements || _isLoop)) {
            $arrow.show().removeClass("ui-lightbox-arrow-prev ui-corner-right").addClass("ui-lightbox-arrow-next ui-corner-left").find("span").removeClass("ui-icon-carat-1-w").addClass("ui-icon-carat-1-e");
          } else {
            this.hideArrow();
          }
        } // for Panorama to work in IE7 & IE8			
      ...
      showErrorMessage: function showErrorMessage() {
        ...
        var $again,
            ...
            _errorScreenHeight = _errorScreenSize.height; // use real data
        ...
        $reject = $structure.find("#ui-lightbox-error-footer-reject"); // ‘again’ button give a user a chance to try loading content again
        ...
        $lb.content.empty().append($structure).find("#ui-lightbox-error").width(_errorScreenWidth).height(_errorScreenHeight).end().children().hide(); // because we don’t want to break the animation queue we need to tell
      ...
      updateTitle: function updateTitle() {
        var data = this.data,
            $lb = this.$lightbox,
            _currentElement = data.currentSetElement; // set new label for the title and trim it if it is too long - no scrolling at the moment
        // 20px is a safety distance between text and the close button

        if (_currentElement.title !== "") {
          $lb.title.empty().append(_currentElement.title);
        } else {
          // keep the line height – prevent counter from popping up in the title line
          $lb.title.append("&nbsp;");
        }
      },
      updateTitleWidth: function updateTitleWidth() {
        var $lb = this.$lightbox; // 12px – 2 × border + 2 × padding

        $lb.header.width($lb.content.width() - 12);
      },
      queueHideContent: function queueHideContent(next) {
        var data = this.data,
            $lb = this.$lightbox,
            _currentElement = data.currentSetElement,
            _options = _currentElement.self.options;
        $lb.content.children().fadeOut(_options.animationSpeed, function () {
          $(this).remove();
          next();
        }); // disable panorama

        $lb.panoramaIcon.hide().removeClass("ui-lightbox-panorama-icon-expand ui-lightbox-panorama-icon-shrink");
        data.panoramaOn = false; // hide the map

        this.panoramaHideMap();
      },
      queueShowOverlay: function queueShowOverlay(next) {
        var data = this.data,
            $lb = this.$lightbox,
            _currentElement = data.currentSetElement.self; // let know that lightbox is not ready now

        data.ready = false; // change cursor to default

        this.setCursor(); // show overlay

        $lb.overlay.fadeIn(_currentElement.options.animationSpeed, next);
      },
      queueCenterLightbox: function queueCenterLightbox(next) {
        var $lb = this.$lightbox,
            $root = $lb.root,
      ...
      queueLoadContent: function queueLoadContent(next) {
        // loads appropriate content using right method
        var _loadContentMethod,
            data = this.data,
            _currentSetElement = data.currentSetElement; // assume that there will be no error
      ...
      queueResizeLightbox: function queueResizeLightbox(next) {
        ...
        var _speed,
            ...
            _minimalLightboxSize = data.minimalLightboxSize,
            _minimalLightboxWidth = _minimalLightboxSize.width,
            _minimalLightboxHeight = _minimalLightboxSize.height; // when it is used in context of a queue


        _speed = _options.animationSpeed; // if content is type of image, resize it to fit the screen
        ...
        if (_currentElement.type === "image" && _isError === false) {
          ...
          _img = $lb.content.find("img"); // if scaled size is smaller than the original, show Panorama

          _currentElement.currentWidth = _imageTargetWidth;
          _currentElement.currentHeight = _imageTargetHeight; // scale the image

          _img.width(_imageTargetWidth).height(_imageTargetHeight); // if you use this method in the context of a queue then use animation; otherwise when used in live resize, don’t animate it
        ...
        $lb.root.find("#ui-lightbox-content").animate({
          width: _lightboxTargetWidth
        }, _speed).animate({
          height: _lightboxTargetHeight
        }, _speed).end().animate({
          left: (this.getWindowSize("width") - _lightboxTargetWidth - _padding) / 2
        }, _speed).animate({
          top: (this.getWindowSize("height") - _lightboxTargetHeight - _padding - _headerHeight) / 2
        }, _speed, next);
      },
      queueCenterContent: function queueCenterContent(next) {
        var _content,
            $lb = this.$lightbox,
            $contentContainer = $lb.content,
            $content = $contentContainer.children();

        $contentContainer.children().css({
      ...
      queueShowContent: function queueShowContent(next) {
        var data = this.data,
            $lb = this.$lightbox,
            self = this,
            _currentElement = data.currentSetElement,
            _options = _currentElement.self.options,
            _isError = data.showErrorMessage; // show content

        $lb.content.children().fadeIn(_options.animationSpeed, function () {
      ...
      queueSlideDownHeader: function queueSlideDownHeader(next) {
        var data = this.data,
            $lb = this.$lightbox,
            _options = data.currentSetElement.self.options; // show header

        $lb.header.slideDown(_options.animationSpeed, next); // show and update counter

        this.updateCounter(); // update title

        this.updateTitleWidth();
        this.updateTitle(); // update buttons states

        this.checkButtonsState(); // indicate that animation queue is finshed

        data.ready = true; // if you go from penulimate/second element to the last/first element change cursor to ‘default’
        // must be after ‘data.ready = true’!!!

        this.setCursor(); // show arrow cue whenever possible when there is no mouse mouvement

        this.showArrow();
      },
      queueSlideUpHeader: function queueSlideUpHeader(next) {
        var data = this.data,
            $lb = this.$lightbox,
            _currentElement = data.currentSetElement,
            _options = _currentElement.self.options; // structure is not ready - start an animation

        data.ready = false; // hide arrow cue

        this.hideArrow(); // change cursor to default
      ...
      data: {
        ...
        htmlFlash: "" + "<object classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' width='{width}' height='{height}'>" + "<param name='movie' value='{url}' />" + "<!--[if !IE]>-->" + "<object type='application/x-shockwave-flash' data='{url}' width='{width}' height='{height}'>" + "<!--<![endif]-->" + "<!--[if !IE]>-->" + "</object>" + "<!--<![endif]-->" + "</object>",
...
/***/ 9:
/*!***************************************************!*\
  !*** multi ./resources/js/jquery.ui.rlightbox.js ***!
  \***************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /Applications/MAMP/htdocs/alejandrozorita.me/resources/js/jquery.ui.rlightbox.js */"./resources/js/jquery.ui.rlightbox.js");
