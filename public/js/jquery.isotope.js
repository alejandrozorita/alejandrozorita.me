...
(function (window, $, undefined) {
  ...
  var Modernizr = window.Modernizr; // helper function
  ...
  var getStyleProperty = function getStyleProperty(propName) {
    var style = document.documentElement.style,
        prefixed; // test standard property first
    ...
    propName = capitalize(propName); // test vendor specific properties
  ...
  var transformProp = getStyleProperty('transform'),
      transitionProp = getStyleProperty('transitionProperty'); // ========================= miniModernizr ===============================
  ...
  */
  ...
  var tests = {
    ...
    csstransforms3d: function csstransforms3d() {
      var test = !!getStyleProperty('perspective'); // double check for Chrome's false positive
  ...
  if (Modernizr.csstransforms) {
    ...
    var setIsoTransform = function setIsoTransform(elem, name, value) {
      ...
      var data = $.data(elem, 'isoTransform') || {},
          ...
          fnName,
          transformObj = {},
          transformValue; // i.e. newData.scale = 0.5

      newData[name] = value; // extend new value over current data
      ...
      valueFns = translateFn + scaleFn; // set data back in elem

      $.data(elem, 'isoTransform', data); // set name to vendor specific property
  ...
  $.Isotope.prototype = {
    ...
    _create: function _create(options) {
      ...
      this.styleQueue = [];
      this.elemCount = 0; // get original styles in case we re-apply them in .destroy()

      var elemStyle = this.element[0].style;
      this.originalStyle = {}; // keep track of container styles
      ...
      this._updateUsingTransforms(); // sorting
      ...
      this.options.getSortData = $.extend(this.options.getSortData, originalOrderSorter); // need to get atoms
    ...
    option: function option(opts) {
      ...
      if ($.isPlainObject(opts)) {
        this.options = $.extend(true, this.options, opts); // trigger _updateOptionName if it exists
    ...
    _updateAnimationEngine: function _updateAnimationEngine() {
      ...
      var isUsingJQueryAnimation; // set applyStyleFnName
    ...
    _updateUsingTransforms: function _updateUsingTransforms() {
      var usingTransforms = this.usingTransforms = this.options.transformsEnabled && Modernizr.csstransforms && Modernizr.csstransitions && !this.isUsingJQueryAnimation; // prevent scales when transforms are disabled
    ...
    updateSortData: function updateSortData($atoms, isIncrementingElemCount) {
      ...
      $atoms.each(function () {
        $this = $(this);
        sortData = {}; // get value for sort data based on fn( $elem ) passed in
    ...
    _sort: function _sort() {
      var sortBy = this.options.sortBy,
        ...
        var a = getSorter(alpha, sortBy),
            b = getSorter(beta, sortBy); // fall back to original order if data matches
    ...
    layout: function layout($elems, callback) {
      var layoutMode = this.options.layoutMode; // layout logic

      this['_' + layoutMode + 'Layout']($elems); // set the size of the container
    ...
    _processStyleQueue: function _processStyleQueue($elems, callback) {
      ...
      var styleFn = !this.isLaidOut ? 'css' : this.isUsingJQueryAnimation ? 'animate' : 'css',
          ...
          processor,
          triggerCallbackNow,
          callbackFn; // default styleQueue processor, may be overwritten down below
      ...
      if (this._isInserting && this.isUsingJQueryAnimation) {
        ...
        callbacks = [callback, onLayout, animOpts.complete],
            instance = this;
        triggerCallbackNow = true; // trigger callback only once
        ...
        if (this.isUsingJQueryAnimation && styleFn === 'animate') {
          ...
          var i = 0,
              ...
              styleObj; // get first non-empty jQ object

          while (!testElem || !testElem.length) {
            styleObj = this.styleQueue[i++]; // HACK: sometimes styleQueue[i] is undefined
          ...
          if (duration > 0) {
            processor = function processor(i, obj) {
              obj.$el[styleFn](obj.style, animOpts) // trigger callback at transition end
    ...
    addItems: function addItems($content, callback) {
      var $newAtoms = this._getAtoms($content); // add new atoms to atoms pools
    ...
    _addHideAppended: function _addHideAppended($newAtoms) {
      ...
      this._isInserting = true; // apply hidden styles
    ...
    _revealAppended: function _revealAppended($newAtoms, callback) {
      var instance = this; // apply visible style after a sec

      setTimeout(function () {
        // enable animation
        $newAtoms.removeClass('no-transition'); // reveal newly inserted filtered elements
    ...
    remove: function remove($content, callback) {
      ...
      this.$filteredAtoms = this.$filteredAtoms.not($content); // remove() as a callback, for after transition / animation
    ...
    _getSegments: function _getSegments(isRows) {
      ...
      segmentSize = this.options[namespace] && this.options[namespace][measure] || // or use the size of the first item, i.e. outerWidth
      this.$filteredAtoms['outer' + capitalize(size)](true) || // if there's no items, use size of container
      containerSize;
      segments = Math.floor(containerSize / segmentSize);
      segments = Math.max(segments, 1); // i.e. this.masonry.cols = ....

      this[namespace][segmentsName] = segments; // i.e. this.masonry.columnWidth = ...
    ...
    _checkIfSegmentsChanged: function _checkIfSegmentsChanged(isRows) {
      var namespace = this.options.layoutMode,
          segmentsName = isRows ? 'rows' : 'cols',
          prevSegments = this[namespace][segmentsName]; // update cols/rows

      this._getSegments(isRows); // return if updated cols/rows is not equal to previous
      ...
      $elems.each(function () {
        ...
        if (colSpan === 1) {
          ...
          var groupCount = props.cols + 1 - colSpan,
              groupY = [],
              groupColY,
              i; // for each group potential horizontal position

          for (i = 0; i < groupCount; i++) {
            // make an array of colY values for that one group
            groupColY = props.colYs.slice(i, i + colSpan); // and get the max value of the array
    ...
    _masonryPlaceBrick: function _masonryPlaceBrick($brick, setY) {
      ...
      var minimumY = Math.min.apply(Math, setY),
          shortCol = 0; // Find index of short column, the first from the left
      ...
      this._pushPosition($brick, x, y); // apply setHeight to necessary columns
    ...
    _masonryHorizontalLayout: function _masonryHorizontalLayout($elems) {
      ...
      $elems.each(function () {
        ...
        if (rowSpan === 1) {
          ...
          for (i = 0; i < groupCount; i++) {
            // make an array of colY values for that one group
            groupRowX = props.rowXs.slice(i, i + rowSpan); // and get the max value of the array
    ...
    _masonryHorizontalPlaceBrick: function _masonryHorizontalPlaceBrick($brick, setX) {
      ...
      var minimumX = Math.min.apply(Math, setX),
          smallRow = 0; // Find index of smallest row, the first from the top
...
/***/ 6:
/*!**********************************************!*\
  !*** multi ./resources/js/jquery.isotope.js ***!
  \**********************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! /Applications/MAMP/htdocs/alejandrozorita.me/resources/js/jquery.isotope.js */"./resources/js/jquery.isotope.js");
